SELECT
    ...
    CASE WHEN b.bed_id IS NULL THEN 'Discharge' ELSE 'Admitted' END AS patient_admission_status,
    -- Doctor Info
...
FROM
    patient p
LEFT JOIN
    satisfaction_score s ON p.patient_id = s.patient_id
LEFT JOIN
    surgery sur ON sur.patient_id = p.patient_id
LEFT JOIN
    hospital_bills hb ON hb.patient_id = p.patient_id
LEFT JOIN
    beds b ON b.patient_id = p.patient_id
LEFT JOIN
    rooms r ON r.room_id = b.room_id
LEFT JOIN
    department dep ON dep.department_id = r.department_id
LEFT JOIN
    (
...
LEFT JOIN
    doctor dr ON dr.doctor_id = a.doctor_id;